...
export const openSubject = (id: string, getToken: TokenGetter) => apiFetch<SubjectView>(`/api/subjects/${encodeURIComponent(id)}`, getToken);
export const listSources = (id: string, getToken: TokenGetter) => apiFetch<StudentSource[]>(`/api/subjects/${encodeURIComponent(id)}/sources`, getToken);
...
export const pageImage = (subjectId: string, globalIndex: number, getToken: TokenGetter) =>
  apiFetchBlob(`/api/subjects/${encodeURIComponent(subjectId)}/pages/${globalIndex}/image`, getToken);
