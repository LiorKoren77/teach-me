...
- Clerk 7 dropped the plan's assumed API. There is no `<SignedIn>`/`<SignedOut>` in this app - `app/page.tsx` and every hook branch on `useAuth()`'s `isLoaded`/`isSignedIn` instead. `createRouteMatcher` is deprecated in Clerk Core 3 because path matching in `proxy.ts` can diverge from how Next.js actually routes a request, so route protection is done twice: `proxy.ts` makes an optimistic `auth.protect()` call for anything under `/learn` or `/admin`, and each of those pages is a server component that calls `await auth.protect()` itself (`/admin` also checks the `role` claim) rather than trusting the matcher.
- Next.js 16 renamed Middleware to Proxy: the file is `proxy.ts` at the repo root (not `middleware.ts`), exporting `clerkMiddleware(...)` as its default export.
- The language preference is exposed through `hooks/useLanguage.ts` as a `useSyncExternalStore` external store (cookie as source of truth, `storage` events as the subscription, `"en"` as the server snapshot) rather than component state, so the server-rendered snapshot and the first client render agree without a flash.
- Page thumbnails are fetched with the session token rather than linked directly: `GET /api/subjects/{subject_id}/pages/{global_index}/image` accepts only a bearer token, which a plain `<img src>` cannot send, so `hooks/usePageImage.ts` fetches each page through the authenticated API client and hands the `<img>` an object URL made from the resulting blob.
- Failed requests go through one mapping rather than being surfaced ad hoc: `lib/api/errors.ts` maps an `ApiError`'s status (401/403/409/429/other) to a translated key in `lib/i18n.ts`'s `errors` block, `hooks/useApiError.ts` sends the raw `detail` to `console.debug` for whoever reads logs, and specifically for 401 it calls Clerk's `redirectToSignIn()` instead of rendering a message the reader can do nothing about.
- The dialog shows the feedback for the answer just graded together with the next question (`AnswerResult` carries both `feedback` and `next_question` in one response) rather than one after the other as the spec's wording suggests - holding the question back would cost a round trip and a second wait for nothing.
- `e2e/learn.spec.ts` reports itself skipped, not failed, when `CLERK_SECRET_KEY` or the `E2E_CLERK_USER_USERNAME`/`E2E_CLERK_USER_PASSWORD` pair is missing (`test.skip(...)`), so `npm run e2e` succeeds trivially in any environment with no Clerk test credentials configured.
- `npx playwright install chromium` needs outbound access to Microsoft's and Google's CDNs specifically; a sandbox that only allows the npm registry cannot reach them, so the one-time browser install (not the test run itself) is the step that needs real network access.
